    ...
    *******************************/
    ...
    DrawingManager: function (map, options) {
        ...
        *******************************/
        ...
        var shapeLayer = new Microsoft.Maps.Layer(),
            edittingLayer = new Microsoft.Maps.Layer(),
            currentShape,
            currentLocation,
            coordIdx,
            isDrawing = false,
        ...
        var _options = {
            ...
            toolbarContainer: {element: null, reload : false },
        ...
        *******************************/
        ...
        function init() {
            ...
            var s = map.layers.indexOf(shapeLayer);
            if (s == -1)
            {
                map.layers.insert(shapeLayer);
            }
            else
            {
            ...
            var e = map.layers.indexOf(shapeLayer);
            if (e == -1) {
                map.layers.insert(edittingLayer);
            }
            else {
                map.layers.removeAt(s);
                map.layers.insert(edittingLayer);
        ...
        * Public Methods
        *******************************/
        this.RemoveToolbarLayer = function () {
        ...
        this.enableRectangletool = function () {
            /// <summary>Gets the options for the drawing tools.</summary>
            /// <returns type='Object(DrawingToolOptions)'></returns>
            self.setDrawingMode('rectangle');
        };
        
        this.disableRectangletool = function () {
            /// <summary>Gets the options for the drawing tools.</summary>
            /// <returns type='Object(DrawingToolOptions)'></returns>
        ...
        this.setDrawingMode = function (mode, handleEsc) {
            ...
            if (drawingMode) {
                if (drawingMode == "erase") {
                    for (var i = 0; i < shapeLayer.getPrimitives().length ; i++) {
                        var s = shapeLayer.getPrimitives()[i];
        ...
        *******************/
        ...
        function createPolyDragHandles() {
            ...
            for (var i = 0; i < len; i++) {
                ...
                editEventIds.push(Microsoft.Maps.Events.addHandler(dragPin, 'dragstart', function (e) {
                    for (var i = 0; i < edittingLayer.getPrimitives().length ; i++) {
                        var s = edittingLayer.getPrimitives()[i];
        ...
        function createCircleDragHandles() {
            ...
            if (currentShape.ShapeInfo.center && currentShape.ShapeInfo.radius != null) {
                ...
                edittingLayer.add(dragRadiusPin);
                ...
                edittingLayer.add(dragCenterPin);
            ...
            if (_options.toolbarContainer) {
                ...
                if (_options.toolbarOptions && _options.toolbarOptions.drawingModes
                    ...
                    for (var i = 0; i < modes.length; i++) {
                       ...
                       // var rel = document.setAttribute("rel", modes[i]);
                       // rel.value = modes[i];
                ...
                if (_options.toolbarContainer.reload)
                {
                    var toolbarlist = _options.toolbarContainer.element;   // Get the <ul> element with id="myList"
                    var childelement = toolbarlist.firstElementChild;
                    toolbarlist.removeChild(childelement)
                    toolbarlist.appendChild(toolbar);
                }
                else
                {
                    _options.toolbarContainer.element.appendChild(toolbar);
                }
...
Microsoft.Maps.moduleLoaded('DrawingToolsModuleV8');